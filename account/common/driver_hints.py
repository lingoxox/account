...
class Hints(object):
    ...
    * ``case_sensitive``: whsolar any comparison should take account of
                          case
    * ``type``: will always be 'filter'
    ...
    def set_sort(self, marker, sort_key, sort_dir):
        """(chengwen):Set sort keywords for pagination"""
        self.marker = marker
        if sort_key:
            self.sort_keys = sort_key.split(',')
        if sort_dir:
            self.sort_dirs = sort_dir.split(',')
